...
async function main() {
	...
	if (!ethers.utils.isAddress(delegatee)) delegatee = wallet.address;
	...
	console.log(
		`Delegatee has a balance of ${ethers.utils.formatEther(
			tokenBalanceDelegatee
		)}`
	);
	let delegateeVotingPower = await tokenContract.getVotes(delegatee);
	console.log(
		`Delegatee voting power is ${ethers.utils.formatEther(
