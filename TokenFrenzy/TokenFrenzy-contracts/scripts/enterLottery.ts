...
async function main() {
	// CLI arguments: 1.contract address 2.token address 3.amount of token (in ETH units) 4.network (goerli or mumbai)
	...
	if (!ethers.utils.isAddress(tokenAddress))
		throw new Error(`Invalid parameter: ${tokenAddress} not a valid address!`);

	const amountTokens = args[2];
	...
	const tx = await lotteryContract.enterLottery(
		tokenAddress,
		BigNumber.from(ethers.utils.parseEther(amountTokens))
	);
